...
extern uint8_t far		control_auto[ MAX_OUTS ];
extern uint8_t far		switch_status_back[ MAX_OUTS ];
void control_input();
void control_output();
...
extern uint8_t get_max_internal_input();
extern uint8_t get_max_internal_output();
S8_T check_external_in_on_line( uint8_t index );
S8_T check_external_out_on_line( uint8_t index );
extern uint8_t change_value_by_range( uint8_t channel );
